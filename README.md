...
- **Ventas de cada día en la planilla:** cuando el script del TPV sube un día de Ágora, Supabase crea sola una fila de ingresos en Compras (Detalle "Ventas del día", Tipo "Ventas", Desde "Ágora", Hacia "Terrys", Ingresos = total cobrado con IVA, y en Información la cantidad de tickets y el desglose por forma de pago). Van azuladas con la etiqueta **Ágora**, no se editan ni se borran (si el día se vuelve a subir, se actualizan solas) y el filtro "Ocultar ventas" las saca de la vista. Si un mes tiene además una fila manual de facturación, la página avisa para no contarla dos veces.
