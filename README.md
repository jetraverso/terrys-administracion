...
Página de administración general de Terry's Burgers. Tiene tres solapas: **Finanzas** (ingresos, egresos y facturas a proveedores de Terrys Burgers S.L., mes a mes; reemplaza al Google Sheet), **Ventas** (las ventas diarias que llegan de Ágora) y **Reportes** (en qué se va la plata). Es una sola página (`index.html`) que guarda todo en Supabase y pide email y contraseña para entrar.
...
- **Ventas de cada día en la planilla:** cuando el script del TPV sube un día de Ágora, Supabase crea sola una fila de ingresos en Finanzas (Detalle "Ventas del día", Tipo "Ventas", Desde "Ágora", Hacia "Terrys", Ingresos = total cobrado con IVA, y en el Detalle la cantidad de tickets y el desglose por forma de pago: `Ventas del día: 24 tickets · Tarjeta 413,90 · Efectivo 40,20 · Glovo 36,64`). Van azuladas con la etiqueta **Ágora**, no se editan ni se borran (si el día se vuelve a subir, se actualizan solas) y el filtro "Ocultar ventas" las saca de la vista. Si un mes tiene además una fila manual de facturación, la página avisa para no contarla dos veces.
- **Buscar y filtrar** (por pagar, sin factura, sin Clara, ocultar ventas), **duplicar** una fila (para gastos que se repiten), **borrar** con "Deshacer".
